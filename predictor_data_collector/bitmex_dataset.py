...
class BitmexDataset:
    ...
    def minutes_of_new_data(self, symbol, kline_size, data, source):
        ...
        if len(data) > 0:
            ...
            old = \
                self.bitmex_client.Trade.Trade_getBucketed(
                    symbol=symbol,
                    binSize=kline_size,
                    count=1,
    ...
    def get_all_bitmex(self, symbol, kline_size, save=False):
        ...
        if rounds > 0:
            ...
            for round_num in tqdm(range(rounds)):
                time.sleep(3)
                new_time = (
                    oldest_point + timedelta(minutes=round_num * self.batch_size * self.BINSIZES[kline_size])
                )
            ...
            print(data_df.head())   # (Optional, used for debugging)
