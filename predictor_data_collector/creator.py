...
def preprocess(dataset, cfg, logger=None):
    ...
    try:
        ...
        if logger is not None:
            logger.error("Error: Low and High columns not found in dataset.")
    ...
    indicators_names = list(cfg.indicators_names.split(' '))
    ...
    dataset, profit_calculator = create_dataset(arr,
                                                list(dates),
...
def create_dataset(dataset, dates, look_back, features):
    ...
    if not all(column in data_frame.columns for column in required_columns):
        raise ValueError(f"Error: Required columns {required_columns} not found in dataset.")

    profit_calculator = data_frame.copy()[required_columns]
